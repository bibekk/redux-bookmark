import React from 'react';
import '../App.css';
import {useSelector, useDispatch} from 'react-redux'
import allActions from '../actions'
import AddCategory from './addcat'
import CategoryList from '../components/categorylist'
import {Divider, Modal, Header} from 'semantic-ui-react'

const Managecat = (props)=> {
    const categories = useSelector(state => state.categories)
    //const isLoading = useSelector(state => state.itemsIsLoading)
    const hasErrored  = useSelector(state=> state.itemsHasErrored)
    const dispatch = useDispatch()

    if(hasErrored){
        return (
            <p>Error!!</p>
        )
    }

    return (
    <Modal open={props.open} size='large' closeIcon onClose={props.close}>
        <Header icon='list alternate outline' content='Manage Category'/>
        <Modal.Content>
            <Modal.Description>
            <AddCategory/>
            <Divider />
            <CategoryList categories={categories} deleteCat={(id)=>dispatch(allActions.deleteCat(id))} 
                editCat= {(id)=>dispatch(allActions.editCat(id))} updateCat={(id,cat)=>dispatch(allActions.updateCat(id,cat))} 
                cancelCatEdit={(id)=>dispatch(allActions.cancelCatEdit(id))} />
            </Modal.Description>
        </Modal.Content>
    </Modal>   
    );
}

export default Managecat
